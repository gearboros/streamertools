...
fn get_state_view(state: &PredictionState) -> Element<'static, Message, Theme, Renderer> {
    if state.phase.is_none() {
        ...
        column![Text::new("Voting active, currently at:"), get_points_distribution(&state.current_state)]
            .spacing(SPACING).into()
    } else if state.phase == Some(PredictionStatus::Locked) {
        column![Text::new("Voting closed, prediction active."), get_points_distribution(&state.current_state)]
            .spacing(SPACING).into()
...
fn get_points_distribution(state: &Option<CreatePredictionResponseData>) -> Element<'static, Message, Theme, Renderer> {
    ...
    let mut col: Column<_> = column![Text::new(format!("Total: {} points & {} users", total_points, total_users))]
        .spacing(SPACING);
...
impl App {
    ...
    pub(crate) fn get_prediction_tab_content(&self) -> Element<'static, Message, Theme, Renderer> {
        ...
        if state.phase == Some(PredictionStatus::Active) || state.phase == Some(PredictionStatus::Locked) {
            cancel_btn = cancel_btn.on_press(Message::Prediction(PredictionMessage::CancelPrediction));
        }
        let mut reset_btn = button("Reset");
        if state.phase == Some(PredictionStatus::Resolved) || state.phase == Some(PredictionStatus::Canceled) {
